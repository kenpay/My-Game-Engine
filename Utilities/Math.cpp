...
{
	return GLfloat(sqrt(((double) x)*((double) x) + double(y)*((double) y)));
}
...
Vector3 & Vector3::operator = (Vector4 & vector)
{
	Vector3 &thisObject = *this;
	thisObject.x = vector.x;
	thisObject.y = vector.y;
	thisObject.z = vector.z;
	return thisObject;
}
...
{
	double _x = double(x), _y = double(y), _z = double(z), _w = double(w);
